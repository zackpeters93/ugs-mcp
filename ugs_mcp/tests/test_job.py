...
from ugs_mcp.tools.job import tool_run_file, tool_cancel_job, tool_pause_job, tool_get_job_status
...
async def test_run_file_preview_returns_warning(tmp_path):
    f = tmp_path / "test.nc"
    f.write_text("G21\nG90\nM3 S18000\nG1 X10 F500\nM5\n")
    with patch("ugs_mcp.tools.job.get_status", new=AsyncMock(return_value=_IDLE_STATUS)):
        result = await tool_run_file(str(f))
...
async def test_pause_job_sends_command():
    ...
    with patch("ugs_mcp.tools.job.send_gcode", new=send_mock):
