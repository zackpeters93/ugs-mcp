...
async def test_jog_preview_returns_banner():
    with patch("tools.motion.get_status", new=AsyncMock(return_value=_IDLE_STATUS)):
...
async def test_jog_blocked_in_alarm_state():
    with patch("tools.motion.get_status", new=AsyncMock(return_value=_ALARM_STATUS)):
...
async def test_return_to_zero_preview_returns_banner():
    result = await tool_return_to_zero(confirmed=False)
    assert "WARNING" in result
